function extractTextRecursively(doc) {
  const body = doc.body;
  ...
  iframes.forEach(iframe => {
    ...
    textContent += extractTextRecursively(iframeDoc);
  });

  const shadowHosts = body.querySelectorAll("*");
  shadowHosts.forEach(host => {
    if (host.shadowRoot) {
        textContent += extractTextRecursively(host.shadowRoot);
    }
  });
...
function filter() {
  const bodyText = (window.location.href + " " + document.title + " " + extractTextRecursively(document)).toLowerCase();
